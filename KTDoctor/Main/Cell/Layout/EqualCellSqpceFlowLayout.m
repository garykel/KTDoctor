...
@interface EqualCellSqpceFlowLayout(){
    //在居中对齐的时候需要知道这行所有cell的宽度总和
    CGFloat _sumCellWidth ;
    NSInteger _sumColumnCount;
    UICollectionViewLayoutAttributes *firstAttr;
}
@end

@implementation EqualCellSqpceFlowLayout
-(instancetype)init{
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (NSUInteger index = 0; index < layoutAttributes.count ; index++) {
        ...
        if (index == 0) {
            _sumColumnCount = (long)floorf(rect.size.width / currentAttr.frame.size.width);
            firstAttr = layoutAttributes[0];
        }
        //加入临时数组
...
-(void)setCellFrameWith:(NSMutableArray*)layoutAttributes{
    ...
    //如果当前行的列数不足最大列数时居左对齐
    if (layoutAttributes.count < _sumColumnCount) {
    ...
    switch (_cellType) {
        case AlignWithLeft:
            nowWidth = firstAttr.frame.origin.x;
