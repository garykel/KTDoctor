//
//  UpdatePowerTemplateViewController.m
//  KTDoctor
//
...
@interface UpdatePowerTemplateViewController ()
<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,XXTGDropdownMenuDelegate>
...
- (void)configHeaderview {
    ...
    if (typeList.count > 2) {
        NSDictionary *dict = [typeList objectAtIndex:1];
        NSString *name = [dict valueForKey:@"name"];
        [self.trainingPositionMenu.mainBtn setTitle:name forState:UIControlStateNormal];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (rotationAngleArr.count > 0) {
        ...
        NSArray *leftAngles = [leftAngle componentsSeparatedByString:@"°"];
        if (leftAngles.count > 0) {
            leftAngle = leftAngles[0];
        }
        NSString *rightAngle = rotationAngleArr[1];
        NSArray *rightAngles = [rightAngle componentsSeparatedByString:@"°"];
        if (rightAngles.count > 0) {
            rightAngle = rightAngles[0];
        }
...
- (void)computeWeight {
    if (self.groups.count > 0) {
        ...
        self.trainingVolumeLbl.text = [self getTrainingVolumeString:sumWeight];
    }
...
- (void)updateTemplate:(NSMutableDictionary *)parameter {
    __weak typeof (self)weakSelf = self;
    [[NetworkService sharedInstance] requestWithUrl:[NSString stringWithFormat:@"%@%@",kSERVER_URL,kDOCTOR_TEMPLATE_UPDATE_URL] andParams:parameter andSucceed:^(NSDictionary *responseObject) {
        NSInteger code = [[responseObject valueForKey:@"code"] longValue];
        NSString *msg = [responseObject valueForKey:@"msg"];
