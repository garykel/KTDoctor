...
@property (nonatomic,assign)NSInteger selectedTemplateType2;//选中模板类型 1强度 2功率
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.selectedTemplateType2 == 2) {
...
- (void)saveAsCustomTemplate:(UIButton*)sender {
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:@"保存" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        ...
        if (nameTF.text.length == 0) {
            ...
            [parameter setValue:@(self.selectedTemplateType2) forKey:@"type2"]; //类型2，1=强度，2=功率
...
- (void)dropdownMenu:(KTDropDownMenus *)menu selectedCellStr:(NSString *)string
{
    ...
    if (menu == self.trainingPositionMenu) { //训练部位
        ...
        self.selectedTemplateType2 = [[self.selectedTemplateDict valueForKey:@"type2"] integerValue];
        if (self.selectedTemplateType2 == 1) {
        ...
        if ([self.trainingPositionMenu.mainBtn.titleLabel.text isEqualToString:@"请选择"]) {
            ...
            if ([self.recommendTemplateArr count] == 0) {
                ...
                if (self.selectedTemplateType2 == 1) {
                    ...
                    self.avgDifficultyLbl.text = @"平均功率：0 w";
                }
                [self.listView reloadData];
            }
        }
...
- (void)computeAvgDifficulty {
    if (self.groups.count > 0) {
        ...
        if (self.selectedTemplateType2 == 1) {
            ...
            self.avgDifficultyLbl.text = [NSString stringWithFormat:@"平均功率：%d w",sumDifficulty / self.groups.count];
        }
    }
}
