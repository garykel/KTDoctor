...
- (void)headerViewButtonHandler {
    ...
    self.headerView.resetButtonHandlerBlock =^ {
        __strong PGDatePickManager *strong_self = weak_self;
        if (strong_self.delegate && [strong_self.delegate respondsToSelector:@selector(resetDatePicker)]) {
            [strong_self.delegate resetDatePicker];
        }
    };
}
